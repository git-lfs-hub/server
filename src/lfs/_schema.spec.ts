...
describe('lockVerifyRequestSchema', () => {
  ...
  test('accepts cursor and limit', () => {
    expect(lockVerifyRequestSchema.safeParse({ cursor: 'abc123', limit: 25 }).success).toBe(true);
  });

  test('accepts ref', () => {
    expect(lockVerifyRequestSchema.safeParse({ ref: { name: 'refs/heads/main' } }).success).toBe(
      true,
    );
  });
...
describe('lockVerifyResponseSchema', () => {
  test('accepts { ours, theirs } with no next_cursor', () => {
    expect(lockVerifyResponseSchema.safeParse({ ours: [LOCK], theirs: [] }).success).toBe(true);
  });
