...
describe('presignR2ObjectUrl (PUT)', () => {
  ...
  test('href targets the configured S3 endpoint', async () => {
    expect(parse(await presignPut(KEY, S3.S3_PRESIGN_TTL)).host).toBe(
      'test-account.r2.cloudflarestorage.com',
    );
  });

  test('href path contains bucket name then key', async () => {
    expect(parse(await presignPut(KEY, S3.S3_PRESIGN_TTL)).pathname).toBe(
      '/lfs-objects/alice/repo/abc123def456',
    );
  });

  test('href uses AWS Signature Version 4', async () => {
    expect(parse(await presignPut(KEY, S3.S3_PRESIGN_TTL)).params.get('X-Amz-Algorithm')).toBe(
      'AWS4-HMAC-SHA256',
    );
  });

  test('href X-Amz-Expires matches S3_PRESIGN_TTL', async () => {
    expect(parse(await presignPut(KEY, S3.S3_PRESIGN_TTL)).params.get('X-Amz-Expires')).toBe(
      S3.S3_PRESIGN_TTL,
    );
  });
  ...
  test('href credential contains the configured access key ID', async () => {
    const credential =
      parse(await presignPut(KEY, S3.S3_PRESIGN_TTL)).params.get('X-Amz-Credential') ?? '';
...
describe('presignUpload', () => {
  test('verify.href is the passed verifyHref', async () => {
    const {
      actions: { verify },
    } = (await new ObjectsStorage(ENV).presignUpload(KEY, VERIFY_HREF)) as {
      actions: { verify: { href: string } };
    };
...
describe('presignDownload', () => {
  ...
  test('download href is an HTTPS URL', async () => {
    const result = (await new ObjectsStorage(envWithObject).presignDownload(KEY)) as {
  ...
  test('download href path contains bucket name then key', async () => {
    ...
    expect(parse(result.actions.download.href).pathname).toBe(
      '/lfs-objects/alice/repo/abc123def456',
    );
  });
});
