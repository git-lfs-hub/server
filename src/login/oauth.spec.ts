...
describe('GET /authorize', () => {
  ...
  test('substitutes GITHUB_CLIENT_ID', async () => {
    const res = await get('/authorize?redirect_uri=http://127.0.0.1:8080/&state=s');
    const location = new URL(res.headers.get('Location')!);
  ...
  test('sets redirect_uri to our own callback URL', async () => {
    const res = await get('/authorize?redirect_uri=http://127.0.0.1:8080/&state=s');
    const location = new URL(res.headers.get('Location')!);
    expect(location.searchParams.get('redirect_uri')).toMatch(/\/login\/oauth\/callback$/);
  });

  test('forwards scope', async () => {
    const res = await get('/authorize?redirect_uri=http://127.0.0.1:8080/&scope=repo,gist&state=s');
    const location = new URL(res.headers.get('Location')!);
  ...
  test('forwards login hint when present', async () => {
    const res = await get('/authorize?redirect_uri=http://127.0.0.1:8080/&state=s&login=alice');
    const location = new URL(res.headers.get('Location')!);
  ...
  test('seals an empty client_state when state query is absent', async () => {
    ...
    expect(res.status).toBe(302);
    const signedState = new URL(res.headers.get('Location')!).searchParams.get('state')!;

    mockGitHub({ access_token: 'ghu_token' });
    const cb = await get(`/callback?code=gh_code&state=${encodeURIComponent(signedState)}`);
...
describe('GET /callback', () => {
  ...
  test('sends our client_id and client_secret to GitHub for code exchange', async () => {
    ...
    await get(`/callback?code=gh_code&state=${encodeURIComponent(signedState)}`);
