...
describe('GET /api/v3/meta', () => {
  ...
  test('installed_version satisfies GCM minimum for OAuth (>= 3.2.0)', async () => {
    const body = (await (await proxyApi.request('/v3/meta')).json()) as any;
    const [major, minor] = (body.installed_version as string).split('.').map(Number);
...
function mockGitHub(body: unknown, status = 200, extraHeaders: Record<string, string> = {}) {
...
function capturedHeaders(spy: ReturnType<typeof vi.spyOn>): Record<string, string> {
...
describe('GET /api/v3/user', () => {
  ...
  test('sends required GitHub API headers', async () => {
    ...
    expect(headers['User-Agent']).toBe('git-lfs-hub');
